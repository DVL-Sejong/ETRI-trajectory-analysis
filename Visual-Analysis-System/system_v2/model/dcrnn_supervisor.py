...
class DCRNNSupervisor(object):
    ...
    def evaluate(self, sess, **kwargs):
        ...
        scaler = self._data['scaler']
        scaler2 = self._data['scaler2']
        predictions = []
        y_truths = []
        for horizon_i in range(self._data['y_test'].shape[1]):
            y_truth = scaler2.inverse_transform(self._data['y_test'][:, horizon_i, :, 0])
            y_truth = scaler.inverse_transform(self._data['y_test'][:, horizon_i, :, 0])
            y_truths.append(y_truth)

            y_pred = scaler2.inverse_transform(y_preds[:y_truth.shape[0], horizon_i, :, 0])
